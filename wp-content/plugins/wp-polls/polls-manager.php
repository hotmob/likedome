		...
		case __('Edit Poll', 'wp-polls'):
			...
			if($get_polla_aids) {
				...
				foreach($polla_aids as $polla_aid) {
					$polla_answers = addslashes(trim($_POST['polla_aid-'.$polla_aid.'-0']));
                                        $poll_answers_custom = addslashes(trim($_POST['polla_aid-'.$polla_aid.'-1']));
                                        $poll_answers_image = addslashes(trim($_POST['polla_aid-'.$polla_aid.'-2']));
					$polla_votes = intval($_POST['polla_votes-'.$polla_aid]);
					$edit_poll_answer = $wpdb->query("UPDATE $wpdb->pollsa SET polla_answers = '$polla_answers', polla_votes = $polla_votes, poll_answers_custom = '$poll_answers_custom',poll_answers_image = '$poll_answers_image' WHERE polla_qid = $pollq_id AND polla_aid = $polla_aid");
					if(!$edit_poll_answer) {
			...
			if(!empty($polla_answers_new)) {
				...
				$polla_answers_new_votes = $_POST['polla_answers_new_votes'];
                                $polla_answers_custom_new = $_POST['polla_answers_custom_new'];
                                $polla_answers_image_new = $_POST['polla_answers_image_new'];
                               
				foreach($polla_answers_new as $key=>$item) {
					$polla_answer_new = addslashes(trim($item));
                                        $polla_answer_custom_new = addslashes(trim($polla_answers_custom_new[$key]));
                                        $polla_answer_image_new = addslashes(trim($polla_answers_image_new[$key]));
					if(!empty($polla_answer_new)) {
						$polla_answer_new_vote = intval($polla_answers_new_votes[$i]);
						$add_poll_answers = $wpdb->query("INSERT INTO $wpdb->pollsa VALUES (0, $pollq_id, '$polla_answer_new', $polla_answer_new_vote, '$polla_answer_custom_new', '$polla_answer_image_new')");
						if(!$add_poll_answers) {
...
?>
	...
	case 'edit':
		...
		$poll_answers = $wpdb->get_results("SELECT polla_aid, polla_answers, polla_votes, poll_answers_custom, poll_answers_image FROM $wpdb->pollsa WHERE polla_qid = $poll_id ORDER BY polla_aid ASC");
						...
						if($poll_answers) {
							...
							foreach($poll_answers as $poll_answer) {
								...
								$polla_votes = intval($poll_answer->polla_votes);
                                                                $poll_answers_custom = stripslashes($poll_answer->poll_answers_custom);
                                                                $poll_answers_image = stripslashes($poll_answer->poll_answers_image);
								...
								echo "<td width=\"60%\"><input type=\"text\" size=\"10\" maxlength=\"20\" name=\"polla_aid-$polla_aid-0\" value=\"".htmlspecialchars($polla_answers)."\" /><input type=\"text\" size=\"10\" maxlength=\"20\" name=\"polla_aid-$polla_aid-1\" value=\"".htmlspecialchars($poll_answers_custom)."\" /><input type=\"text\" size=\"50\" maxlength=\"160\" name=\"polla_aid-$polla_aid-2\" value=\"".htmlspecialchars($poll_answers_image)."\" />&nbsp;&nbsp;&nbsp;";
