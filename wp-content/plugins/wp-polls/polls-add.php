	...
	switch($_POST['do']) {
		...
		case __('Add Poll', 'wp-polls'):
			...
			$polla_answers = $_POST['polla_answers'];
                        $polla_answers_custom = $_POST['polla_answers_custom'];
                        $polla_answers_image = $_POST['polla_answers_image'];
			$polla_qid = intval($wpdb->insert_id);
			foreach($polla_answers as $key=>$item) {
				$polla_answer = addslashes(trim($item));
                                $polla_answer_custom = addslashes(trim($polla_answers_custom[$key]));
                                $polla_answer_image = addslashes(trim($polla_answers_image[$key]));
				$add_poll_answers = $wpdb->query("INSERT INTO $wpdb->pollsa VALUES (0, $polla_qid, '$polla_answer', 0, '$polla_answer_custom', '$polla_answer_image')");
				if(!$add_poll_answers) {
	...
	<table class="form-table">
		...
		<?php
			for($i = 1; $i <= $poll_noquestion; $i++) {
				...
				echo "<td width=\"80%\"><input type=\"text\" size=\"10\" maxlength=\"20\" name=\"polla_answers[]\" /><input type=\"text\" size=\"10\" maxlength=\"20\" name=\"polla_answers_custom[]\" /><input type=\"text\" size=\"50\" maxlength=\"160\" name=\"polla_answers_image[]\" />&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"".__('Remove', 'wp-polls')."\" onclick=\"remove_poll_answer_add(".$i.");\" class=\"button\" /></td>\n";
				echo "</tr>\n";
				$count++;
			}
		?>
