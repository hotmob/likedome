        </div>
    </div>
    <div class="clear"></div>
    <div class="footer height-149">
        <div class="wrapper"> <a href="#" class="backTop fro mar">Back to top</a>
            <div class="clear"></div>
            <p><a target="_blank" href="index.html/">网站首页</a> | <a target="_blank" href="#">关于我们</a> | <a target="_blank" href="h#">新闻投递</a> | <a target="_blank" href="商务合作.html">商务合作</a> | <a target="_blank" href="商务合作.html">广告购买</a> | <a target="_blank" href="mailto:renzhong(a)766.com" title="请将邮箱地址中的(a)换成@">主编信箱</a><br>
                版权所有:福州来动网科技有限公司 闽ICP证B2-20100066<br>
                Copyright &copy; www.likedo.cn (2011-2011) All Rights Reserved.</p>
        </div>
    </div>
    </div>
    <div class="server">
        <ul>
            <!-- <li><a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=138545336&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:138545336:45" alt="点击这里给我发消息" title="点击这里给我发消息">在线客服1</a></li>
            <li><a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=139545336&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:139545336:45" alt="点击这里给我发消息" title="点击这里给我发消息">在线客服2</a></li>
            <li><a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=138545336&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:138545336:45" alt="点击这里给我发消息" title="点击这里给我发消息">在线客服3</a></li>
            <li><a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=139545336&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:139545336:45" alt="点击这里给我发消息" title="点击这里给我发消息">在线客服4</a></li> -->
        </ul>
        <p>服务电话：<br />
            暂未开放</p>
    </div>
    <div class="suggest">
        <div class="suggest-con">
            <p>
                <input type="checkbox" />
                <label class="margin-l6">我要匿名发表</label>
            </p>
            <p><span class="margin-l18">匿名发表建议采纳没有奖励哦%>_</%</span></p>
            <textarea class="margin-t10"></textarea>
            <div class="margin-t13">
                <input type="button" value="提交建议" class="btn3" />
                <input type="button" value="重　　置" class="btn3 margin-l18" />
            </div>
            <a href="#" class="suggest-close">关闭</a> </div>
    </div>
<?php wp_footer(); ?>
</body>
</html>
